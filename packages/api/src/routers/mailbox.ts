...
import { jellyMailboxMember, jellyMailbox, marmaladeMailbox, marmaladeMailboxMember } from "@marmalade-v2/db/schema/mailbox";
...
export const mailboxRouter = {
  list: protectedProcedure.handler(async ({ context }) => {
    ...
    .handler(async ({ input, context }) => {
      ...
      const mailbox = await db.select().from(jellyMailbox).where(eq(jellyMailbox.jellyMailboxId, input.jellyMailboxId));
      if (!mailbox) {
        throw new ORPCError("NOT_FOUND", {
          message: "Mailbox not found",
        });
      }
      const newMailbox = await db.insert(marmaladeMailbox).values({
    ...
    resync: publicProcedure.handler(async ({ context }) => {
      ...
      try {
        ...
        const existingMailboxes = await db.select().from(jellyMailbox).where(inArray(jellyMailbox.jellyMailboxId, mailboxIds));
        const existingMailboxIds = existingMailboxes.map(mailbox => mailbox.jellyMailboxId);
        console.log('existingMailboxIds', existingMailboxIds)
        const newMailboxes = mailboxes.filter(mailbox => !existingMailboxIds.includes(mailbox.id));
        const removedMailboxes = existingMailboxes.filter(mailbox => !mailboxIds.includes(mailbox.jellyMailboxId));
        if (removedMailboxes.length > 0) {
          await db.update(jellyMailbox).set({ existsInJelly: false }).where(inArray(jellyMailbox.jellyMailboxId, removedMailboxes.map(mailbox => mailbox.jellyMailboxId)));
        }
        if (newMailboxes.length > 0) {
          await db.insert(jellyMailbox).values(newMailboxes.map(mailbox => ({
        ...
        for (const mailbox of mailboxes) {
          call(mailboxRouter.resyncMembers, {
    ...
    resyncMembers: publicProcedure.input(
      z.object({
      ...
      try {
        ...
        const existingMembers = await db.select().from(jellyMailboxMember).where(inArray(jellyMailboxMember.jellyMemberId, memberIds));
        const existingMemberIds = existingMembers.map(member => member.jellyMemberId);
        console.log('existingMemberIds', existingMemberIds)
        const newMembers = mailboxMembers.filter(member => !existingMemberIds.includes(member.id));
        const removedMembers = existingMembers.filter(member => !memberIds.includes(member.jellyMemberId));
        if (removedMembers.length > 0) {
          await db.delete(jellyMailboxMember).where(inArray(jellyMailboxMember.jellyMemberId, removedMembers.map(member => member.jellyMemberId)));
        }
        if (newMembers.length > 0) {
          await db.insert(jellyMailboxMember).values(newMembers.map(member => ({
